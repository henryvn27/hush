...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\hush-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("hush-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("hush-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/hush-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("hush-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/hush-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("hush-recordings")
    }
