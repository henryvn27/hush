...
- The Flow Bar is now intentionally small: the CSS control is capped at 9.5rem and the native window is 144x40, keeping it close to Wispr's low-interruption pill reference.
...
- The source-rebuilt Tauri app bundle is at `target/release/bundle/macos/Hush.app`; the app passes deep strict code-signature verification. The rebuilt local DMG at `/private/tmp/Hush-0.5.0-aarch64-native.dmg` passes `hdiutil verify` with SHA-256 `e6dcda66390e48e3a549ea9efadd52f6c5a0b96f54fe99338318e50d75278aad`. It is ad hoc signed and not notarized because Apple signing credentials are not configured on this machine.
- The current verified local app ZIP is at /private/tmp/Hush-0.5.0-aarch64-native.zip with SHA-256 9ae36d977d89b7abf336ea6789a70271345fbf902cb98818cab59c521c404e2c. The same artifacts are copied into `frontend/public/downloads/` for the landing page. The app passes deep strict code-signature verification and its DMG passes read-back verification.
