...
- The source-rebuilt Tauri app bundle is at `target/release/bundle/macos/Hush.app`; the app passes deep strict code-signature verification. The rebuilt local DMG at `/private/tmp/Hush-0.5.0-aarch64-runtime.dmg` passes `hdiutil verify` with SHA-256 `e127ad03ecdfccf530797f7af64acbb5e61424dd35dbbb51e6697463b3395f29`. It is ad hoc signed and not notarized because Apple signing credentials are not configured on this machine.
- The current verified local app ZIP is at /private/tmp/Hush-0.5.0-aarch64-runtime.zip with SHA-256 dca0329ffee230c6b06e0bfb32850e4b54f94823a6b827591a83aa46a7435205. The same artifacts are copied into `frontend/public/downloads/` for the landing page. The app passes deep strict code-signature verification and its DMG passes read-back verification.
- The latest frontend production build completed successfully during the native bundle build. The separate Flow Bar now polls backend recording state in its own WebView, uses targeted main-window event delivery with an app-wide fallback, and is covered by regression contracts; native Accessibility insertion, multi-display bar persistence, and authenticated Wispr Hub workflows remain unverified.
